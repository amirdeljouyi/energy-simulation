import { HouseholdStepResult, SimulationStep } from '../types/simulation';

const formatKw = (value: number) => value.toFixed(1);

type HouseholdPlaybackPanelProps = {
  step: SimulationStep | null;
  household: HouseholdStepResult | null;
  isPlaying: boolean;
  speedMs: number;
  onTogglePlay: () => void;
...
export default function HouseholdPlaybackPanel({
  step,
  household,
  isPlaying,
  speedMs,
  ...
  currentIndex,
  totalSteps,
}: HouseholdPlaybackPanelProps) {
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/60">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Household Playback</h3>
          <p className="mt-1 text-sm text-slate-500">Playback focuses on the selected household.</p>
        </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Household power</p>
          <p className="mt-2 text-base font-semibold text-slate-900">
            {formatKw(household?.netLoadKw ?? 0)} kW
          </p>
          <p className="mt-1 text-xs text-slate-500">
            Load {formatKw(household?.loadKw ?? 0)} kW · PV {formatKw(household?.pvKw ?? 0)} kW
          </p>
        </div>
      </div>
    </div>
